              ...
              {items.slice(0, limit).map((item) => {
                ...
                return (
                  <div key={item.id} className="group flex items-center gap-3 py-2.5">
                    ...
                    >
                      {file ? (
                        <FileThumb fileId={item.id} category={item.category} alt={item.originalFilename} hasThumbnail={Boolean(item.thumbnailUrl)} src={item.thumbnailSrc} className="h-9 w-9" />
                      ) : (
